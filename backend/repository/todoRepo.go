...
func (repo TodoRepository) Create(ctx context.Context, todo *domain.Todo) error {
	...
	`
	var id string
	err := repo.db.QueryRowContext(ctx, query, todo.Title).Scan(&id)
	if err != nil {
...
func (repo TodoRepository) Update(ctx context.Context, todo *domain.Todo) error {
	query := `
		UPDATE todos
		SET title = $2, active_at = $3, status = $4
		WHERE id = $1
	`
	_, err := repo.db.ExecContext(ctx, query, todo.ID, todo.Title, todo.ActiveAt, todo.Status)
	if err != nil {
